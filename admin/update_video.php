        ...
        <div class="card-body">
            ...
            <form class="forms-sample" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <br>
                    <input type="file" name="video" class="form-control-file" style="margin: 10px 0px 10px 0px;">
                    <p>Current video: <br>
                    <video controls autoplay loop style="margin:10px 0px 10px 0px;" height="300px" width="300px">
...
if(isset($_POST['sub'])) {
    ...
    $folder="uploads/";
    $existing_video_q = "select video_file from video where video_ID='$videoid'";
    $existing_video_result = mysqli_query($connect,$existing_video_q);
    $existing_video_data = mysqli_fetch_assoc($existing_video_result);
    $existing_video = $existing_video_data['video_file'];
    if(!empty($video_name)) {
        if ($video_type == "video/mp4" || $video_type == "video/avi" || $video_type == "video/webm") {
            if ($video_size<=10000000) {
                $path=$folder.$video_name;
                move_uploaded_file($temp_name,$path);
                $q = "update video set title = '$title', artist_ID_FK = '$artistid', album_ID_FK = '$albumid', language_ID_FK = 
                '$languageid', genre_ID_FK = '$genreid' ,video_file='$path', year = '$year', description = '$bio' where video_ID='$videoid'";
            } else {
                echo "<script>alert('Video size should not exceed 10MB'); window.location.href('view_video.php');</script>";
                exit;
            }

        } else {
            echo "<script>alert('only mp4, avi, webm files are allowed'); window.location.href='view_video.php';</script>";
            exit;
        }
    } else {
        $q = "update video set title = '$title', artist_ID_FK = '$artistid', album_ID_FK = '$albumid', language_ID_FK = 
                '$languageid', genre_ID_FK = '$genreid' ,video_file='$existing_video', year = '$year', description = '$bio' where video_ID='$videoid'";
    }
    $result = mysqli_query($connect,$q);
    if ($result) {
      echo "<script>alert('Updated Successfully'); window.location.href='view_video.php';</script>";
    } else {
